...
Nine projects: the three libraries, their three test projects, the two console
tools, and the consumer-shape sample. `global.json` selects the
Microsoft.Testing.Platform runner and pins NO SDK version, matching the rest of
the family - the machine's newest installed SDK is the one that builds.
...
repository root. That file does nothing else - it pins no SDK version - and it
must stay committed: without it `dotnet test` falls back to the older VSTest
bridge. None of the three test projects references a coverage collector, so a
run collects no coverage and needs no extra package to succeed.
...
application registers it; the LIBRARY never does. KEEPING THAT PIN AT THE NEWEST
PUBLISHED OPUS IS THE STANDING RULE: it is bumped whenever a newer package is
published, and it has been bumped more than once on that rule alone. A bump is
expected to change nothing these tests can observe. Packet-loss concealment is
the case in point - the Opus package says whether it can conceal a loss, and
...
Rendering/YuvShaderSource.cs: four EXACT node fetches (the cell's black corner,
...
THE RULE. EVERY FILE HERE IS BUILT THROUGH THE AUTHORING LIBRARY - the generator
asks the library for each command line and CodeBrix.VideoProcessing runs the
host's ffmpeg - and MANIFEST.txt records the line that really produced each file.
A FOLDER IS RE-ENCODED WHEN THE COMMANDS THAT MAKE IT CHANGE, AND LEFT ALONE WHEN
THEY DO NOT. As things stand:

    MKV, WebM, CodeBrix-Mode1   the eighteen ffmpeg-muxed files, REBUILT on
                                2026-09-02, when the `-map_chapters` token became
                                unconditional and every WebM-profile command line
                                therefore changed (see the last note in this file)
    CodeBrix-Mode2              the six bespoke files, standing since 2026-08-29;
                                its two passes did not change on 2026-09-02, so
                                there was nothing to re-encode

There is a test that keeps this honest: CorpusCommandEquivalenceTests in
tests/CodeBrix.VideoPlayback.Authoring.Tests compares the committed manifest's
command lines with what the library renders today, for the three ffmpeg-muxed
folders. Mode2 is not compared - its second half is muxed by managed code and it
was authored by this library from the start. If that test ever fails, the folder
it names has to be rebuilt - which is a decision, taken deliberately, not
something to slip into a commit.
...
- The .slnx lists AGENT-README.txt, CBV-FORMAT.txt, EXTRAS-README.txt,
  ...
  under Solution Items, all three libraries at the top level, all three test
  projects under Tests, the sample under Samples and the tools under Tools.
