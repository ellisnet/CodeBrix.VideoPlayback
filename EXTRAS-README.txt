...
EVERY FILE HERE IS BUILT THROUGH THE AUTHORING LIBRARY, and MANIFEST.txt records
the command line that really produced each one. The eighteen ffmpeg-muxed files -
MKV/, WebM/ and CodeBrix-Mode1/ - were REBUILT on 2026-09-02, when the
`-map_chapters` token became unconditional and every WebM-profile command line
therefore changed. CodeBrix-Mode2/ was not re-encoded then and stands as it was
written on 2026-08-29, because its two passes did not change. A folder is
re-encoded when the commands that make it change and left alone when they do not,
and a test keeps that honest: CorpusCommandEquivalenceTests compares the command
lines the committed manifest records with what the library renders today, for the
three ffmpeg-muxed folders. See MAINTAINER-README.txt.
