...
public class MainViewModel : SimpleViewModel
{
    ...
    /// <summary>Writes the chain that is on screen out as one ".cube" file.</summary>
    public SimpleCommand BakeCommand => field ??= new SimpleCommand(() => CanBakeChain, _ => DoBake());

    private bool CanBakeChain =>
        controller != null
        && LutPanelPolicy.CanBake(controller.LutEntries.Count, controller.TransportState, controller.EffectsActive);

    private void DoBake()
    {
        if (controller == null) { return; }

        BakedLut baked = controller.BakeAppliedChain(BakeLocations.CreateFilePath(DateTime.Now));
    ...
    {
        if (!controller.EffectsActive)
        {
            //Worth saying out loud: on the processor path the chain is applied to the presenter but not to
            //  the picture, so a bake there is the chain's arithmetic and not what is on screen.
            SmokeLog("bake-note the effect chain is not active, so this bake is the chain, not the picture");
        }

        BakedLut baked = controller.BakeAppliedChain(smoke.BakePath);
